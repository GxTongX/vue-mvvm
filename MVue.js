...
class Observer{
    ...
    defineReactive(obj, key, value){
        ...
        Object.defineProperty(obj, key, {
            ...
            get(){
                // 订阅数据变化时，往Dep中添加观察者
                Dep.target && dep.addWatcher(Dep.target);//跑到新建watcher的时候发生的
                return value;
            },
...
const compileUtil = {
    ...
    //绑定观察者watcher，将来数据有变化，将触发这里的回调函数，进行视图更新
    text(node, expr, vm) {
        let value;
        if(expr.indexOf('{{')!==-1){
...
class MVue {
    constructor(options) {
        ...
        if (this.$el) {
            ...
            // 3. 通过数据代理实现 this.person.name = '郭猩猩'功能，而不是this.$data.person.name = 'GxYeah'
